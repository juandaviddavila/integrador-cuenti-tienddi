...
let reondeoCentimos = function (
  ...
  if (!es_decimal) {
    ...
    parte_decimal = Math.round(
      parseFloat(parte_decimal) * Math.pow(10, cantidad_decimales),
    );
    if (parte_decimal === 0) {
...
$.get_data_imagen = async (
  ...
  try {
    ...
    if (rows_imagen !== null) {
      ...
      let SQL =
        "SELECT id_imagen,ext1,ext2,numero_imagen FROM adm_imagenes WHERE id_externo=:id_externo and tipo=1;";
...
$.get_data_compound = async (
  ...
  for (const row of rows) {
    ...
    let product = await $.get_data_product_id(
      id_company,
      data,
      row.id_producto,
      rows_sucursal,
    ...
    lst.push({
      product: product,
      quantity:
        redondeo(parseFloat(row.unidades), 2) < 0
          ? 0
          : redondeo(parseFloat(row.unidades), 2), //validar
...
$.get_data_product_id = async (
  ...
  try {
    conn = await objGestorBd.getConnectionEmpresa(id_company);
    let SQL = objGestorSQL.getSqlNombre(
      "integrador_tienddi",
      "list_product_tienddi",
    );
    ...
    rows = await conn.query2(SQL, {
      id_sucursal: data.id_branch,
      desde: 0,
      hasta: 1,
    });
    console.log(1);
...
$.get_data_products = async (id_company, data) => {
  ...
  try {
    conn = await objGestorBd.getConnectionEmpresa(id_company);
    //validar configuracion sucursal para ver si aplico impuestos y si aplico impuestos al consumo
    let SQLSucursal = objGestorSQL.getSqlNombre(
      "integrador_tienddi",
      "config_sucursal",
    );
    rows_sucursal = await conn.query2(SQLSucursal, {
      id_sucursal: data.id_branch,
    });
    ...
    rows = await conn.query2(SQL, {
      id_sucursal: data.id_branch,
      desde: desde_paginacion,
      total: total_paginacion,
    });

    let SQlCategoria = objGestorSQL.getSqlNombre(
      "integrador_tienddi",
      "list_categorias",
    );
    ...
    SQL =
      "SELECT id_imagen,ext1,ext2,numero_imagen,id_externo FROM adm_imagenes WHERE tipo=1 GROUP BY id_externo ;";
  ...
  rows_sucursal.vender_con_impuestos = parseInt(
    rows_sucursal.vender_con_impuestos,
  );
  rows_sucursal.vender_impuestos_agregado = parseInt(
    rows_sucursal.vender_impuestos_agregado,
  );
  rows_sucursal.activar_venta_compra_licores = parseInt(
    rows_sucursal.activar_venta_compra_licores,
  );
  rows_sucursal.vender_ip_estampilla = parseInt(
    rows_sucursal.vender_ip_estampilla,
  );
  let lst = [];
  for (const row of rows) {
    if (row.nombre !== null) {
...
$.generate_product = async function (
  id_company,
  data,
  row,
  rows_categoria,
  ...
  let aux_imagen = null;
  aux_imagen = await $.get_data_imagen(
    id_company,
    row.id_producto,
    data.imagen_base64,
    rows_imagen,
  ...
  row.precio_venta_online =
    row.precio_venta_online === null ? 0 : row.precio_venta_online;
  ...
  if (
    !(
      rows_sucursal.activar_venta_compra_licores === 1 &&
      rows_sucursal.vender_ip_estampilla === 1
    )
  ) {
  ...
  if (
    rows_sucursal.vender_con_impuestos === 0 &&
    rows_sucursal.vender_impuestos_agregado === 0
  ) {
  ...
  if (
    rows_sucursal.vender_con_impuestos === 0 &&
    rows_sucursal.vender_impuestos_agregado === 1
  ) {
    ...
    if (rows_categoria[i].id_categoria === row.id_categoria) {
      if (rows_categoria[i].id_categoria_padre === null) {
        ...
        for (let j = 0; j < rows_categoria.length; j++) {
          if (
            rows_categoria[j].id_categoria ===
            rows_categoria[i].id_categoria_padre
          ) {
  ...
  let obj = {
    ...
    stock:
      redondeo(parseFloat(row.existencias), 2) < 0
        ? 0
        : redondeo(parseFloat(row.existencias), 2), //existencias validar
    ...
    inventory_control:
      row.controla_inventario_tienda_linea === 1 ? true : false, //realiza control de inventario si es true no dejar vendar sin existenacia y si es false dejar vender sin existencias
    ...
    if (row.configuracion_dinamica === null) {
      ...
      for (const conf of row.configuracion_dinamica) {
        let modelo = {
          title: conf.titulo,
          multiple_choices:
            conf.permitirMarcarMultiplesOpciones == 1 ? true : false, //permitirMarcarMultiplesOpciones aplica solo si type es opcion_multiple, este permite que cada opcion se puede seleccionar mas cantidades
          ...
          type:
            conf.tipo === "opcion"
              ? "option"
              : "option_multiple" /*lista de opciones opcion(option)/opcion_multiple(option_multiple)*/,
          options: [],
        };
        for (const opcion of conf.opciones) {
          ...
          modelo.options.push({
            ...
            quantity:
              redondeo(parseFloat(opcion.cantidad), 2) < 0
                ? 0
                : redondeo(parseFloat(opcion.cantidad), 2), //cantidad de item que saca del inventario validar
      ...
      //sacar ingredientes
      obj.configuration_compound = await $.get_data_compound(
        id_company,
        data,
        row.id_producto,
        rows_sucursal,
    ...
    if (row.presentaciones !== null) {
      ...
      for (const presentacion of row.presentaciones) {
        ...
        for (let j = 0; j < 10; j++) {
          ...
          if (
            presentacion["precio" + (j + 1) + "_online"] == 0 &&
            presentacion["precio" + (j + 1)] > 0
          ) {
            presentacion["precio" + (j + 1) + "_online"] =
              presentacion["precio" + (j + 1)];
          }
          ...
          if (presentacion["cantHas" + (j + 2)] !== undefined) {
            if (
              presentacion["cantHas" + (j + 1)] >
              presentacion["cantHas" + (j + 2)]
            ) {
              break;
            }
          }
...
$.getMetricas = async (id_company, data) => {
  ...
  try {
    ...
    for (let row_detalle of rows) {
      ...
      row_detalle.costo_actual = parseFloat(
        row_detalle.costo_actual * row_detalle.cantidad,
      );
    }
    ...
    if (data_cache === null) {
      ...
      if (fecha_minima != null) {
        if (fecha_minima <= fecha_corte) {
          ...
          const fechaMinimaFormateada = fecha_minima
            .toISOString()
            .split("T")[0];
          SQL = SQL.replaceAll(
            ":fecha_minima",
            "AND d._date>='" + fechaMinimaFormateada + "'",
          );
        }
      } else {
        SQL = SQL.replaceAll(":fecha_minima", "");
      ...
      for (let row_detalle of row) {
        ...
        row_detalle.total_price =
          row_detalle.total_price === null ? 0 : row_detalle.total_price;
        row_detalle.avg_price =
          row_detalle.avg_price === null ? 0 : row_detalle.avg_price;
        row_detalle.avg_discount =
          row_detalle.avg_discount === null ? 0 : row_detalle.avg_discount;
        row_detalle.num_discounts =
          row_detalle.num_discounts === null ? 0 : row_detalle.num_discounts;
        row_detalle.num_promotions =
          row_detalle.num_promotions === null ? 0 : row_detalle.num_promotions;
        row_detalle.total_price2 =
          row_detalle.total_price2 === null ? 0 : row_detalle.total_price2;
...
var uuidv4 = function (id_empresa, tipoDocumento, id_usuario) {
  return (
    ...
    "-" +
    id_usuario +
    "-xxxxxxxx-xxxx-xxx".replace(/[xy]/g, function (c) {
...
$.getPaymentLink = async (data) => {
  ...
  try {
    ...
    conn = await objGestorBd.getPool_bases();
    let SQL =
      "SELECT id,id_empresa,id_transaccion,codigo,valor,es_activo,`url`,x_gtm,id_sucursal FROM boton_pago_intencion WHERE id_empresa=:id_empresa AND id_transaccion=:id_transaccion AND es_activo=1;";
  ...
  try {
    ...
    let config = {
      ...
      data: {
        ...
        webhookUrl:
          "https://integrador-cuenti-tienddi.cuenti.co/api/v1/integration-free/webhookCuentiPay/" +
          data.codigo,
        param1: "test",
        param2: "",
        param3: "",
...
$.findActiveEvents = async (id_company, branchId, slug) => {
  try {
    ...
    let cache =
      "cache_findActiveEvents_" + id_company + "_" + branchId + "_" + slug;
    let data_cache = await $.getFromCache(cache);
    if (data_cache !== null) {
      console.log("token desde cache:" + data_cache);
      return data_cache;
...
$.activarPagos = async (data) => {
  try {
    ...
    if (lstPagos_pendientes.length > 0) {
      //configuraciones
      lst_configuraciones = await $.getConfigurations({
        id_empresa: data.id_empresa,
        id_sucursal: lstPagos_pendientes[0].id_sucursal,
      });
...
let validacionPagosCuentiPay = async function (
  token,
  lst_configuraciones,
  row,
) {
...
$.webhookCuentiPay = async (_data, codigo) => {
  try {
    ...
    if (data.length > 0) {
      ...
      //validar cada referencia si esta ya esta pagada
      let r = await validacionPagosCuentiPay(
        token,
        lst_configuraciones,
...
$.get_conf_modulos_sucursal = async (id_company, id_sucursal) => {
  ...
  if (data_cache === null) {
    ...
    try {
      conn = await objGestorBd.getConnectionEmpresa(id_company);
      let SQL =
        "SELECT conf_modulos FROM adm_sucursal WHERE  id_sucursal=:id_sucursal;";
...
$.get_conf_modulos_sucursal_generarQr_url_dian = async (
  id_company,
  id_sucursal,
) => {
  ...
  if (conf_modulos !== null && conf_modulos !== undefined) {
    try {
      ...
      if (
        conf_modulos.generarQr_url_dian !== undefined &&
        conf_modulos.generarQr_url_dian !== null
      ) {
...
$.registrarIngresovehiculo = async (
  id_company,
  token,
  id_sucursal,
  id_empleado,
      ...
      data: [
        {
          descuento_global: 0,
          bodega_destino: null,
          activar_factura_electronica_pos: 0,
          id_canal: 1,
          domicilio: 0,
          orden_compra: null,
          id_pedido_tienda: null,
          id_banco_tienda: null,
          pago_relizado_tienda: 0,
          total_estampilla: 0,
          total_impoconsumo: 0,
          turno: null,
          tiempo_aprox_prestamo: null,
          total_gastos_orden_servicios: null,
          marca: null,
          retenciones: [],
          id_cliente_contacto: null,
          codeUnicoQr: codeUnicoQr,
          encabezado: "",
          propinaManual: false,
          iva_sobre_utilidad: false,
          porAdmi: 0,
          porImprovis: 0,
          porUtilidad: 0,
          ...
          objClienteMini: {
            nombre_cliente: data.plate,
            identificacion: data.plate,
            telefono1: "",
            email1: "",
            id: 0,
            direccion: direccion,
...
$.validarSarlidaVh = async (id_company, id_sucursal, placa) => {
  ...
  try {
    ...
    let conf_modulos = await $.get_conf_modulos_sucursal(
      id_company,
      id_sucursal,
    );
    if (conf_modulos !== null && conf_modulos !== undefined) {
      try {
        ...
        try {
          if (
            conf_modulos.placas_excepciones !== undefined &&
            conf_modulos.placas_excepciones !== null
          ) {
            for (let i = 0; i < conf_modulos.placas_excepciones.length; i++) {
              if (
                conf_modulos.placas_excepciones[i].p.trim().toUpperCase() ==
        ...
        if (
          conf_modulos.reglas_gratis !== undefined &&
          conf_modulos.reglas_gratis !== null
        ) {
          try {
            ...
            if (Array.isArray(conf_modulos.reglas_gratis)) {
              reglaDia = conf_modulos.reglas_gratis.find(
                (r) => r.dia === diaActual && r.aplica === true,
              );
            }
    ...
    conn = await objGestorBd.getConnectionEmpresa(id_company);
    let SQL =
      "SELECT id_transacion_ingreso,id_transacion FROM transacion_ingreso_parqueadero p WHERE p.es_activo=1 AND p.id_sucursal=:id_sucursal and placa=:placa;";
      ...
      if (r[0].id_transacion === null) {
        ...
        if (min_gratis > 0) {
          ...
          let r2 = await conn.query2(SQL, {
            id_transacion_ingreso: r[0].id_transacion_ingreso,
            min_gratis: min_gratis,
          });
          if (r2.length > 0) {
...
$.webhook_parking = async (
  ...
  try {
    ...
    if (data.doorway === "entrance") {
      let r = await $.registrarIngresovehiculo(
        id_company,
        token,
        id_sucursal,
        id_empleado,
      ...
      if (r.type == 1) {
        ...
        throw new Error(
          "Error al registrar ingreso de vehículo: " + JSON.stringify(r),
        );
      }
    } else {
...
$.get_data_company = async (id_empresa, id_sucursal) => {
  ...
  try {
    ...
    conn = await objGestorBd.getPool_bases();
    let SQL =
      "SELECT nombre_empresa,nit_empresa,id_empresa,es_multi_empresa,alias FROM empresas WHERE id_empresa=:id_empresa";
    let data_base = (await conn.query2(SQL, { id_empresa: id_empresa }))[0];
    if (data_base.es_multi_empresa == 1) {
      SQL =
        "SELECT nombre_empresa,nit_empresa,id_empresa,es_multi_empresa,id_empresa_padre,alias FROM empresas WHERE id_empresa_padre=:id_empresa AND id_sucursal=:id_sucursal;";
      let data_base_sucursal = await conn.query2(SQL, {
        id_empresa: id_empresa,
        id_sucursal: id_sucursal,
      });
      if (data_base_sucursal.length > 0) {
        if (
          data_base_sucursal[0].nombre_empresa != null &&
          data_base_sucursal[0].nit_empresa !== null
        ) {
          if (
            data_base_sucursal[0].nombre_empresa.toString().trim().length > 0 &&
            data_base_sucursal[0].nit_empresa.toString().trim().length > 0
          ) {
    ...
    //capturara imagen
    SQL =
      "SELECT id_imagen,nombre,bits,tamano,formato,ext1,ext2,id_empresa,id_sucursal FROM imagenes WHERE id_sucursal=:id_sucursal AND id_empresa=:id_empresa;";
    let logo = "";
    let queryForList2 = await conn.query2(SQL, {
      id_empresa: id_empresa,
      id_sucursal: id_sucursal,
    });
    if (queryForList2.length === 0) {
      SQL =
        "SELECT COUNT(*) as total FROM empresas e INNER JOIN imagenes i ON(e.id_imagen=i.id_imagen) WHERE e.id_empresa=:id_empresa LIMIT 1;";
      let countResult = await conn.query2(SQL, { id_empresa: id_empresa });
      if (countResult[0].total > 0) {
        SQL =
          "SELECT i.id_imagen,formato FROM empresas e INNER JOIN imagenes i ON(e.id_imagen=i.id_imagen) WHERE e.id_empresa=:id_empresa LIMIT 1;";
        let queryForList = await conn.query2(SQL, { id_empresa: id_empresa });
        logo =
          queryForList[0].id_imagen +
          ";" +
          (queryForList[0].formato == null
            ? ""
            : queryForList[0].formato.toString());
      } else {
        logo = "";
      }
    } else {
      logo =
        ...
        id_sucursal +
        ";" +
        queryForList2[0].formato.toString();
    }
    if (logo != "") {
...
$.pre_activar_plan = async (id_empresa, id_transaccion) => {
  ...
  try {
    ...
    let cont = await conn.query2(
      "SELECT COUNT(*) FROM empresa_intento_pago_pre_activacion WHERE id_empresa=1  AND fecha_registro >= DATE_SUB(CURDATE(), INTERVAL 10 DAY);",
      { id_empresa: id_empresa },
    );
    if (cont[0]["COUNT(*)"] > 0) {
      return {
        type: 1,
        message: "Ya existe un intento de activacion en los ultimos 10 dias",
      };
    } else {
      //Paso 2 registro en la tabla empresa_intento_pago_pre_activacion
      await conn.query2(
        "INSERT INTO empresa_intento_pago_pre_activacion(id_empresa,fecha_registro,id_transaccion)VALUES(:id_empresa,NOW(),:id_transaccion);",
        { id_empresa: id_empresa, id_transaccion: id_transaccion },
      );
      //Paso 3 registro  UPDATE plan_empresa SET fecha_vencimiento=:fecha_vencimiento WHERE id_empresa=:id_empresa and es_activo=1;
      let lstPlan = await conn.query2(
        "SELECT id_plan_empresa,fecha_vencimiento FROM plan_empresa WHERE es_activo=1 AND id_empresa=:id_empresa;",
        { id_empresa: id_empresa },
      );
      ...
      if (lstPlan.length > 0) {
        ...
        if (fecha_vencimiento_plan < fecha_vencimiento) {
          await conn.query2(
            "UPDATE plan_empresa SET fecha_vencimiento=:fecha_vencimiento WHERE id_plan_empresa=:id_plan_empresa_activo;",
            {
              fecha_vencimiento: fecha_vencimiento,
              id_plan_empresa_activo: id_plan_empresa_activo,
            },
          );
        }
      } else {
        //activo demo 1 dia
        await conn.query2(
          "UPDATE aplicaciones_empresa SET fecha_vencimiento=:fecha_vencimiento WHERE id_empresa=:id_empresa;",
...
$.getId_usuario_id_empleado = async (id_company, id_empleado) => {
  let cache =
    "cache_validar_id_usuario_empleado_" + id_company + "_" + id_empleado;
  ...
  try {
    ...
    await $.storeInCache(
      cache,
      respuesta,
      (ttlInSeconds = tiepo_cache_segundos),
    );
  }
};
$.getPermiso = async (id_empresa, id_usuario, id_permiso) => {
  ...
  try {
    ...
    //Paso 1 valido que si en la tabla empresa_intento_pago_pre_activacion ya existe un registro hace 3 dias termino aqui
    let cont = await conn.query2(
      "SELECT COUNT(*) FROM permisos_usuarios WHERE es_activo=1 AND id_usuario=:id_usuario AND id_empresa=:id_empresa AND id_lista_permisos=:id_lista_permisos;",
      {
        id_empresa: id_empresa,
        id_usuario: id_usuario,
        id_lista_permisos: id_permiso,
      },
    );
    if (cont[0]["COUNT(*)"] > 0) {
...
$.validar_permiso = async (id_company, id_empleado, id_permiso) => {
  ...
  if (respuesta.type == 1) {
    let permiso = await $.getPermiso(
      id_company,
      respuesta.id_usuario_portal,
      id_permiso,
    );
    if (permiso.type == 1) {
      await $.storeInCache(
        cache,
        { type: 1, message: "El usuario tiene permisos" },
        (ttlInSeconds = tiepo_cache_segundos),
      );
      ...
      await $.storeInCache(
        cache,
        { type: 0, message: "El usuario no tiene permisos" },
        (ttlInSeconds = tiepo_cache_segundos),
      );
      return { type: 0, message: "El usuario no tiene permisos" };
    }
    ...
    await $.storeInCache(
      cache,
      { type: 0, message: "No se encontro el empleado" },
      (ttlInSeconds = tiepo_cache_segundos),
    );
...
FROM transacion_encabezado t INNER JOIN adm_cliente c ON (c.id_cliente=t.id_cliente) WHERE t.id_transacion=:id_transacion;`;
      let rows2 = await conn.query2(SQL, {
        id_transacion: rows[0].id_transacion,
      });
      return rows2[0];
    } else {
...
$.validarCrearColumna = async (
  ...
  try {
    ...
    //Paso 1 valido que si en la tabla empresa_intento_pago_pre_activacion ya existe un registro hace 3 dias termino aqui
    let cont = await conn.query2(
      `SELECT COUNT(*) FROM ejecucion_validacion_columna_creada 
WHERE id_empresa=:id_empresa AND nombre_columna=:nombre_columna 
AND nombre_tabla=:nombre_tabla AND id_aplicacion=:id_aplicacion;`,
      {
        id_empresa: id_empresa,
...
$.registrarCrearColumna = async (
  ...
  try {
    ...
    //Paso 1 valido que si en la tabla empresa_intento_pago_pre_activacion ya existe un registro hace 3 dias termino aqui
    await conn.query2(
      `INSERT INTO ejecucion_validacion_columna_creada(id_empresa,nombre_columna,nombre_tabla,id_aplicacion,fecha_registro)
...
$.valiadarRangosDeFechasDeVentasCierreCaja = async (
  id_company,
  id_empleado,
  id_sucursal,
) => {
  let cache =
    "cache_validacion_rangos_fechas_cierre_caja_" +
    id_company +
    "_" +
    id_empleado +
    "_" +
  ...
  try {
    ...
    if (
      (
        await $.validarCrearColumna(
          id_company,
          "transacion_encabezado_ext",
          "es_conciliado",
          2,
        )
      ).type == 0
      ...
      if (rows_existe.length == 0) {
        ...
        await conn.query2(
          `ALTER TABLE transacion_encabezado_ext
      ...
      ADD FOREIGN KEY (id_conciliacio_pago) REFERENCES transacion_conciliacion_pagos (id_conciliacio_pago);`,
        ...
        await conn.query2(
          `UPDATE  transacion_encabezado_ext SET es_conciliado=1;`,
          {},
        );
        await $.registrarCrearColumna(
          id_company,
          "transacion_encabezado_ext",
          "es_conciliado",
          2,
...
WHERE ext.es_conciliado=0 AND t.id_empleado=:id_empleado AND t.es_nula=0 and t.id_sucursal=:id_sucursal;`;
    ...
    let row = await conn.query2(SQL, {
      id_sucursal: id_sucursal,
      id_empleado: id_empleado,
    });
    await $.storeInCache(
      cache,
      { sw_crear_columna: sw_crear_columna, data: row },
      (ttlInSeconds = 60 * 1),
    ); //1 min
...
$.get_personalizacion_sucursal = async (id_company, id_sucursal) => {
  let cache =
    "cache_personalizado_modulos_sucursal_" + id_company + "_" + id_sucursal;
  let data_cache = await $.getFromCache(cache);
  let r = null;
  if (data_cache === null) {
    let conn = null;
    try {
      conn = await objGestorBd.getConnectionEmpresa(id_company);
      let SQL =
        "SELECT personalizado FROM adm_sucursal WHERE  id_sucursal=:id_sucursal;";
      r = await conn.query2(SQL, { id_sucursal: id_sucursal });
...
$.activar_impuesto_consumo_lujo = async (
  id_company,
  id_sucursal,
  es_activo,
) => {
  ...
  try {
    conn = await objGestorBd.getConnectionEmpresa(id_company);
    let SQL =
      "UPDATE adm_sucursal SET personalizado=:personalizado WHERE id_sucursal=:id_sucursal;";
    if (es_activo == 0) {
      ...
      let p = {
        impuestos_personalizados: {
          impuesto_consumo: {
            impuesto_1: {
              tipo: "valor",
              nombre: "Impoconsumo licores y cigarrillos",
              nombre_corto: "Impo",
              nombre_corto2: "I-Impo",
              descripcion: "Impoconsumo licores y cigarrillos",
              nombre_campo: "total_impoconsumo",
              nombre_combinado: "Impoconsumo + Estampilla",
              abreviatura: "I",
              valor_basico: 0,
              nombre_combinado2: "Impoconsumo + ICUI",
            },
            impuesto_2: {
              tipo: "valor",
              nombre: "Estampillas",
              ...
              nombre_campo: "total_estampilla",
              nombre_combinado: "Impoconsumo + Estampilla",
              abreviatura: "E",
              valor_basico: 0,
              nombre_combinado2: "Estampilla + IBUA",
            },
          },
          impuesto_personalizado: {
            impuesto_1: {
              tipo: "porcentaje",
              ...
              nombre_corto2: "INC",
              descripcion: "IC",
              nombre_campo: "Impuesto Nacional al Consumo",
              nombre_combinado: "INC",
              abreviatura: "INC",
              valor_basico: 8,
              nombre_combinado2: "Impuesto Nacional al Consumo",
            },
            impuesto_2: {
              tipo: "valor",
              nombre: "IBUA",
              nombre_corto: "IBUA",
              nombre_corto2: "B-IBUA",
              descripcion: "IBUA ULTRAPROCESADOS BEBIDAS",
              nombre_campo: "total_estampilla",
              nombre_combinado: "ICUI + IBUA",
              abreviatura: "B",
              valor_basico: 0,
              nombre_combinado2: "Estampilla + IBUA",
            },
...
$.get_consecutivos = async (id_company) => {
  ...
  if (data_cache === null) {
    ...
    try {
      conn = await objGestorBd.getConnectionEmpresa(id_company);
      let SQL =
        "SELECT id_consecutivo,es_activo,id_sucursal FROM adm_consecutivos WHERE es_activo=1;";
      r = await conn.query2(SQL, {});
      return r;
    } catch (error) {
      console.error(error);
...
$.get_empresa_contingencia = async (id_empresa) => {
  let conn = null;
  try {
    console.log("traer conexion");
    conn = await objGestorBd.getPool_bases();
    let SQL = `SELECT id_empresa,nombre_empresa,nit_empresa,id_sucursal FROM empresas 
WHERE id_empresa=:id_empresa OR id_empresa_padre=:id_empresa AND es_activo=1 GROUP BY nit_empresa;`;
    const rows = await conn.query2(SQL, {
      id_empresa: id_empresa,
    });
    return rows;
...
$.get_consecutivosSistemaContingencia = async (id_company) => {
  let conn = null;
  try {
    conn = await objGestorBd.getConnectionEmpresa(id_company);
    let SQL =
