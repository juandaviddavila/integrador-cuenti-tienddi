...
router.post("/get_data_products", async function (req, res) {
  try {
    let r = await objIntegratorTienddiBl.get_data_products(
...
router.post("/get_category", async function (req, res) {
  try {
    let r = await objIntegratorTienddiBl.get_category(
      req.headers["id-company"],
    );
    res.json(r);
  } catch (e) {
...
router.post(
  "/webhook_parking/:id_company/:id_sucursal/:id_empleado",
  queue_express({
    activeLimit: 1,
    queuedLimit: 30,
  ...
  async function (req, res) {
    try {
      // Espera 5 segundos
      // await delay(5000);
      let r = await objIntegratorTienddiBl.webhook_parking(
        req.headers["id-company"],
...
router.post(
  ...
  async function (req, res) {
    try {
      ...
      let r = await objIntegratorTienddiBl.buscarTransacionTag(
        req.headers["id-company"],
        req.body.nombre,
      );
      res.json(r);
...
router.get("/lista_empresas", async function (req, res) {
  try {
    if (req.headers["id-company"] == 2) {
      let r = await objIntegratorTienddiBl.lista_empresas();
      res.json(r);
    } else {
      res.json(null);
    }
...
router.post("/consultar_empresa_sucursal_moneda", async function (req, res) {
  try {
    let r = await objIntegratorTienddiBl.consultar_empresa_sucursal_moneda(
      req.headers["id-company"],
      req.body.id_sucursal,
    );
    res.json(r);
...
router.post("/consultarConsecutivosAndEmpleado", async function (req, res) {
  try {
    let r = await objIntegratorTienddiBl.consultarConsecutivosAndEmpleado(
      req.headers["id-company"],
...
router.get(
  ...
  async function (req, res) {
    try {
      let r = await objIntegratorTienddiBl.listaProvedoresIsa(
        req.headers["id-company"],
      );
      res.json(r);
    } catch (e) {
