...
p.nota,m.nombre_marca,u.nombre AS unidad_medida,i.nombre_impuesto,c.nombre_categoria,prov.nombre_cliente AS nombre_proveedor,prov.identificacion AS identificacion_proveedor
...
LEFT JOIN adm_cliente prov ON(prov.id_cliente=ps.id_proveedor)
 WHERE  es_ingrediente=0 AND ps.es_activo=1 AND p.mostrar_tienda_linea=1 AND p.id_padre IS NULL AND ps.id_sucursal=:id_sucursal 
  ORDER BY p.id_producto LIMIT :desde,:hasta;
